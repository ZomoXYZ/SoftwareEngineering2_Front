...
func _ready():
	#First I check if this is a single player game or not, if so then I remove_at lobby ID and replace it with offline
	...
	for cardWrapper in $Background/HandBox.get_children():
		cardWrapper.connect("pressed_with_val",Callable(self,"_on_Card_pressed"))
...
func _on_Resume_pressed():
	#Same as above but in reverse order, play the opacity change, then hide it so that buttons checked main can be clicked
